...
def main(page: str, limit: int, data_dir: Path): # download specific page, validate page & save each revision in a folder structure based on year & month
    """
    ...
    print(f"Done! {count_revisions(data_dir/page)} revisions downloaded!") # You should call count_revisions() here and print the number of revisions
...
def count_revisions(path_to_subfolder): # make in progress
    ...
    for item in os.listdir(path_to_subfolder):
        ...
        if os.path.isdir(item_path):
            count += count_revisions(item_path)  # Recursive call for subdirectories
        else:
